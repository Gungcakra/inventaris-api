...
router.get("/:id", (req, res) => {
    db.query("SELECT * FROM cabang WHERE id = ?", [req.params.id], (err, result) => {
        if (err) return res.status(500).json({ error: err.message });
...
router.post("/", (req, res) => {
    ...
    db.query("INSERT INTO cabang (name, address) VALUES (?, ?)", [name, address], (err, result) => {
        if (err) return res.status(500).json({ error: err.message });
...
router.put("/:id", (req, res) => {
    const { name, address } = req.body;
    db.query("UPDATE cabang SET name = ?, address = ? WHERE id = ?", [name, address, req.params.id], (err, result) => {
        if (err) return res.status(500).json({ error: err.message });

...
router.get("/:id", (req, res) => {
  db.query(
    "SELECT * FROM pre_order WHERE id = ?",
    [req.params.id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
      if (result.length === 0)
...
router.post("/", (req, res) => {
  ...
  db.query(
    "INSERT INTO pre_order (id_cabang, id_vendor, id_barang, qty, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())",
    [id_cabang, id_vendor, id_barang, qty, status],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
...
router.delete("/:id", (req, res) => {
  db.query(
    "DELETE FROM pre_order WHERE id = ?",
    [req.params.id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });

...
router.post("/", (req, res) => {
  ...
  db.query(
    "INSERT INTO barang (id_vendor, id_cabang, name, price, stock) VALUES (?, ?, ?, ?, ?)",
    [id_vendor, id_cabang, name, price, stock],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
...
router.put("/:id", (req, res) => {
  const { id_vendor, id_cabang, name, price, stock } = req.body;
  db.query(
    "UPDATE barang SET id_vendor=?, id_cabang=?, name=?, price=?, stock=? WHERE id=?",
    [id_vendor, id_cabang, name, price, stock, req.params.id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
